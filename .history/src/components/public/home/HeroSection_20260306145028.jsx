...
const HeroSection = () => {
  ...
  // ២. បង្កើត Logic សម្រាប់ដូរ Background រាល់ ១ នាទី (៦០០០០ មីលីវិនាទី)
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentBg((prev) => (prev + 1) % backgrounds.length);
    }, 60000); 

    return () => clearInterval(interval); // បោសសម្អាត interval ពេល component unmount
  }, [backgrounds.length]);
  ...
  return (
    ...
    <div className="flex items-center justify-center min-h-screen bg-stone-900 p-4">
      ...
      {/* បន្ថែម Custom Animation ក្នុង Tailwind config ឬប្រើ Inline Style */}
      <style jsx>{`
        @keyframes wiggle {
