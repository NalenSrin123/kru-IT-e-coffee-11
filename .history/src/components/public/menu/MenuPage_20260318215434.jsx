...
const MenuPage = () => {
  ...
  return (
    <div className="min-h-screen text-gray-800 ">
      ...
      <div className='w-[100%] sm:w-[90%]  m-auto rounded-3xl mb-6 bg-gradient-to-r from-orange-900/40 via-orange-900/10 to-orange-900/40'>
      <div className='w-[100%] h-[250px] rounded-3xl mt-2 bg-amber-200 m-auto overflow-hidden  border-[3px] border-orange-950'>
      ...
      <main className="w-[95%] m-auto md:p-6 grid grid-cols-1 md:grid-cols-4 gap-7">
        
        {/* --- Sidebar (Filter) --- */}
        <aside className="md:col-span-1 space-y-8 p-6 rounded-2xl h-fit shadow-md bg-gray-200 ">
          <div>
            ...
            <div className="space-y-3 text-sm">
              {['All Drinks', 'Hot Coffee', 'Cold Drinks', 'Frappe Drinks', 'Tea & Refreshers', 'Specialty Drinks'].map((cat) => (
                <label key={cat} className="flex items-center gap-3 cursor-pointer hover:text-amber-700 transition-colors">
                  <input type="checkbox" className="rounded border-gray-600 text-amber-600 focus:ring-amber-500" defaultChecked={cat === 'All Drinks'} />
                  {cat}
                </label>
              ))}
            </div>
